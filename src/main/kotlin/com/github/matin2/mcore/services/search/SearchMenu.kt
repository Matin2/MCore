...
import com.github.matin2.mcore.services.koinOf
...
suspend inline fun Player.openSearch(items: List<ItemStack>) = openSearch({ it.value }) { input ->
	...
	else items.withIndex().asFlow().filter { it.value matches input }
}
...
internal class SearchMenu<T : Any>(
	...
	private val manager: SearchMenuService by inject()
