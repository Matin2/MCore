...
object PacketService : KoinComponent {
	
	private val packetEvents: PacketEventsHook by inject()
	...
	var InventoryView.displayTitle: Component
		...
		set(value) {
			...
			InventoryTitle.openWindows[user]?.let {
				it.title = value
				packetEvents.api?.playerManager?.sendPacket(user, it)
	...
	infix fun Player.sendPacket(packet: PacketWrapper<*>) =
		packetEvents.api?.playerManager?.sendPacket(this, packet)
