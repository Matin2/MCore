...
class AttackAnimation {
    ...
    fun play(player: Player, handType: HandType) {
                ...
                .runTaskAsynchronously(
                    ...
                    Runnable {
                        ...
                        if (handType == HandType.MAINHAND) {
                            sendMainhandMovement(playersInRange, player)
                        } else if (handType == HandType.OFFHAND) {
                            sendOffhandMovement(playersInRange, player)
                        }
                    })
        }
    }
    ...
    private fun filterOutOfRange(
        ...
        for (player in players) {
            ...
            if (playerLocation.world == location.world && (playerLocation.distanceSquared(location) <= range) && predicate.test(player)
            ) {
