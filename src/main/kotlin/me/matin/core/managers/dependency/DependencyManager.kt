...
object DependencyManager {
    ...
    @JvmStatic
    fun checkDepends(plugins: Set<String>, action: (String, CheckedDepend) -> Unit, listener: Plugin? = null) {
        plugins.forEach {
            action(it, if (isPluginInstalled(it)) CheckedDepend.INSTALLED else CheckedDepend.NOT_INSTALLED)
        }
        if (listener != null) Bukkit.getPluginManager().registerEvents(DependencyListener(plugins, action), listener)
    }
    ...
    @JvmStatic
    fun checkDepends(plugin_versions: Map<String, String>, action: (String, CheckedDepend) -> Unit, listener: Plugin? = null) {
        for (name in plugin_versions.keys) {
            val versions = plugin_versions[name]!!
            if (versions.isBlank()) {
        ...
        if (listener != null) Bukkit.getPluginManager().registerEvents(DependencyListener(plugin_versions, action), listener)
    }
